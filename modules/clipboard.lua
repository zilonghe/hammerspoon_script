...
local casplock = require "modules/hotkey"
...
function storeCopy()
   ...
   -- print(last_change)
   -- print(hs.inspect(hs.pasteboard.allContentTypes()))
   if (now > last_change) then
      print(hs.image.imageFromPath("public.png"))
      current_clipboard = pasteboard.getContents()
      -- print(hs.inspect(hs.pasteboard.allContentTypes()))
      -- print(hs.inspect(pasteboard.contentTypes()))
      -- print(type(pasteboard.readImage()))
      -- print(hs.image.imageFromPath(hs.pasteboard.readURL():match("^file://(.*)$")))
      -- print(type(hs.pasteboard.readURL():match("^file://(.*)$")))
      -- print(type(current_clipboard))
      if pcall(isCopyFile) then
         print(pasteboard.readURL():match("^file://(.*)$"))
         current_clipboard = pasteboard.readURL():match("^file://(.*)$")
...
function mod.init()
   if mod.config.use_chooser then
      ...
      hs.hotkey.bind({"cmd", "shift"}, "v", 
         function()
            mod.selectorobj:popupMenu(hs.mouse.getAbsolutePosition()) 
         end)
   end
